...
| DET-CI-SHA | **Périmée en grande partie** (relu le 2026-07-26) : toutes les actions GitHub de `ci.yml` et `release.yml` sont épinglées par SHA, et actionlint est en `:1.7.12` et non `:latest`. Ce qui flotte encore : le tag Docker d'actionlint, qu'un amont peut réécrire, et la bande `dotnet-version: '10.0.x'` | 1 | 2 | 1 | 15 | Épingler actionlint par digest (`docker://…@sha256:…`) ; pour le SDK, un `global.json` fermerait le sujet en même temps que DET-SDK |
