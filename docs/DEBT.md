...
bornés par `global.json`. **830 tests** à la v1.0.0, dont un rejeu de fixtures qui exerce le
chemin complet du scan sans machine Windows.

## Corrigé

Une ligne par dette fermée : ce qu'elle était, et en une phrase ce qui l'a close. **Le récit
complet — ce qui a été mesuré, ce qui a été écarté et pourquoi — est dans
[CHANGELOG.md](../CHANGELOG.md)**, et il n'a pas à être écrit deux fois. Ce tableau a porté
jusqu'à 4 700 caractères dans une seule cellule ; un registre qu'on ne peut plus parcourir
cesse d'être un registre.
...
| DET-PROGRAM | `Program.cs` monolithe : **1 881 lignes** non vides à l'audit contre ~1 240 à l'inscription, +52 % en trois lots, dispatch et 16 commandes mêlés au rendu et au parsing | 2026-07-27, ADR-005 étapes 1 et 2 — 29 à la fermeture, **40 lignes non vides** aujourd'hui : l'encodage console, le refus d'une ligne que l'outil ne sait pas honorer, un appel à la table, le `try/catch` qui traduit une exception en code de sortie. Le reste est dans 17 classes de commande, une table explicite, un hôte pour les auxiliaires et deux surfaces pures dans Core. **Ce qui la fermait n'était pas la taille mais l'absence de filet** : rien ne surveillait 1 400 lignes qu'on s'apprêtait à déplacer. D'où l'ordre suivi — figer le rendu des trois commandes qui écrivent sur la console, extraire ce qui était pur, *puis* déplacer. Dix gardes surveillent la table, tous vérifiés par mutation ; deux d'entre eux ne sont nés que parce que la relecture a montré que les premiers comparaient deux listes écrites de la même main |
| DET-SORTIE-PARTIELLE | `rempart scan` rendait **0** quand les collecteurs avaient lu correctement mais que des règles étaient revenues `Unknown` : le code de sortie était décidé par le statut des collecteurs, jamais par les verdicts. Un scan non élevé dont des contrôles sont illisibles était, pour un appelant qui ne lit que le code — planificateur, script de parc —, indistinguable d'un scan complet | 2026-07-27 — code **5, « audit partiel »**, distinct du 3 : le 3 dit qu'un *collecteur* a été refusé, le 5 qu'une *règle* n'a pas pu être évaluée. Précédence `1 > 3 > 5 > 0`, classée par ce que l'appelant peut en faire. Mesuré sur les quatre fixtures |
| DET-FIXTURE-MATERIEL | Les fixtures « synthétiques » ne sont pas fabriquées de rien : ce sont des captures réelles dont les champs identifiants sont lavés. Il restait, dans un dépôt public, **11 chemins de tâches planifiées** nommant des logiciels installés (`\Launch Adobe CCXProcess`, `\AMDRyzenMasterSDKTask`, `\NVIDIA App SelfUpdate_{GUID}`, `\StartCN`, `\StartDVR`, `\SoftLanding\…`), le **modèle de carte mère** (MS-7E80, PRO B850-S WIFI6E) et la **version et la date du BIOS** (2.A41, 03/17/2026). `Versioned_fixtures_are_anonymised` ne pouvait pas le voir : il ne vérifiait qu'un booléen et le préfixe du nom de machine | 2026-07-27 — **l'anonymiseur étendu plutôt que la phrase corrigée** : identité matérielle et tâches hors `\Microsoft\` hachées, `Hash` rendu idempotent, drapeau désormais produit et non affirmé. Aucun marqueur d'intrusion abîmé |
...
| DET-CATALOGUE-MUET | `CatalogSignature.Verify` rendait `null` pour deux choses différentes — « aucun catalogue ne référence ce fichier », qui est une réponse, et « l'API n'a pas pu être interrogée », qui n'en est pas une — et le jugement rendait `Unsigned` dans les deux cas, donc un constat **`Suspicious`**. Un fichier illisible était **accusé** | 2026-07-27 — `CatalogOutcome` sépare quatre cas ; « personne n'a répondu » tombe sur `Unknown` au lieu d'accuser. Mesuré sur machine réelle : 303 signatures, **aucun verdict déplacé** |
| DET-PORTS-MUET | `IListeningPortProvider.Enumerate` rendait une liste nue : une lecture ratée rendait la même chose qu'une machine sans service exposé, et le rapport concluait « aucun port en écoute », qui se lit comme une bonne nouvelle | 2026-07-27 — `ListeningPortRead` porte `Status` + `Diagnostic`, **à côté** de la liste pour qu'une capture antérieure se relise comme le succès qu'elle était. Quatre tables lues, donc un état `Partial` |
| DET-ARITE-REPORT | `--report` était lu par `OptionalValue` sur `scan` et par `OptionValue` sur `diff`. Sur `rempart diff --report --baseline b.json a.json`, la comparaison partait donc dans un dossier nommé `--baseline` | 2026-07-27 — `diff` lit `--report` comme `scan` l'a toujours fait. Portée réelle du défaut : le dossier de sortie, pas le choix des fichiers. Garde nouveau : une orthographe, une arité, partout |
| DET-EXPLAIN-POSITIONNEL | `explain` lisait son identifiant à l'indice 1 au lieu de passer par `Positional` : `rempart explain --rules <dossier> WIN-CRED-001` listait les contrôles au lieu d'expliquer la règle demandée | 2026-07-27 — passe par `Positional` : 110 lignes de catalogue redeviennent les 28 lignes de la règle demandée. `WordAt` reste juste pour le mot de commande, seul indice fixe qui soit un fait |
| DET-SDK | Pas de `global.json` (SDK non verrouillé) ni de gestion centrale des paquets ; `Microsoft.NET.Test.Sdk`, `xunit` et `xunit.runner.visualstudio` déclarés **deux fois**, dans les deux `.csproj` de test, sans que rien ne relie les copies | 2026-07-27 — `global.json` verrouille **10.0.302** en `rollForward: latestFeature` : plancher à la version de développement, plafond à la fin de 10.0. `Directory.Packages.props` centralise les versions |
| DET-CI-SHA | Le tag Docker d'actionlint (`:1.7.12`) et la bande `dotnet-version: '10.0.x'` flottaient encore, seules exceptions à l'épinglage par empreinte | 2026-07-27 — actionlint épinglé par **digest** d'image. À retenir pour le prochain audit : Dependabot ne rafraîchira jamais ce pin, son analyseur écartant tout `uses:` commençant par `docker://` |
| DET-RECPROV | 13 paires `Recording`/`Snapshot` quasi-identiques, et — ce que l'entrée ne disait pas — **quatre copies de la liste des 20 fournisseurs** : le câblage réel, le câblage d'enregistrement, le câblage de rejeu, et une seconde copie de ce dernier dans la suite de tests | 2026-07-27 — **la généralisation qui payait n'était pas celle qui était écrite** : le générique est impossible sans réflexion. Ce qui coûtait vraiment était quatre copies de la liste des fournisseurs, dont une dans le test censé la surveiller. Trois fabriques nommées, trois gardes |
| DET-PLAGE-DYNAMIQUE | Le premier port de la plage dynamique (49152) était une constante, affirmée de toute machine scannée. Sur une machine reconfigurée, l'outil marquait comme « éphémères » des ports qui ne l'étaient pas, en taisant ceux qui l'étaient — et sans jamais dire qu'il supposait | 2026-07-27 — la plage est **lue de la machine** par un fournisseur, donc enregistrée et rejouable. La constante reste le repli, et le constat dit lequel des deux il a utilisé |
...
| DET-NOTES-AMONT | **120 des 123 notes d'impact du catalogue sont `Upstream`** : rédigées à partir d'une description d'une ligne, jamais confrontées au logiciel installé. Trois seulement sont `Verified`, celles de M5b | Donnée | 2 | 2 | 5 | 4 | Créée volontairement avec la dette qu'elle décrit ([ADR-006](adr/ADR-006-catalogue-bloatware-importe.md), D20) : le champ existe pour que ce chiffre soit **lisible** plutôt que dissimulé. Se réduit d'une unité à chaque logiciel réellement observé — c'est le même mouvement que DET-WINDEFAULT, et il se mesure. Recopié à la main, il a annoncé 113 sur 116 pendant sept entrées ; un test le confronte désormais au catalogue. **Depuis le 2026-08-02, le rapport le dit** : un constat porteur d'une note amont l'annonce en toutes lettres et porte `note: amont` dans ses détails. Le champ ne servait qu'à ce registre ; il rendait au lecteur une description d'une ligne avec l'autorité d'une mesure |
...
| DET-DERIVE-FACTEUR | Une série est dite périmée au-delà de **trois fois** sa cadence observée. Le seuil est mesuré — médiane des intervalles de la série — mais le facteur trois est un choix, posé pour tolérer un intervalle sauté | Donnée | 1 | 2 | 2 | 12 | Même famille que le seuil de fraîcheur des données d'ADR-002, et écrit comme tel dans la spec et dans le code. Aucune série réelle n'existe encore : la première le recalera. Le facteur est tenu des deux côtés par un test (le passer à 2 ou à 4 rougit), donc le changer est un geste visible et non une dérive |
| DET-TLS | Règles SCHANNEL/TLS non livrées : les défauts varient selon la build | Code | 3 | 3 | 4 | 12 | Demande une vérification sur plusieurs machines (ROADMAP M2b). **Le préalable est levé le 2026-08-02** : le collecteur `tls` relève l'état SCHANNEL sans rien juger, donc toute capture prise désormais porte la preuve — elle n'y était dans aucune, une capture n'enregistrant que ce qui a été lu. Première observation, build 26200 : quatorze valeurs sur seize **absentes**, seul `TLS 1.2` côté client posé explicitement (`Enabled=1`, `DisabledByDefault=0`), et rien côté serveur. Un `windowsDefault` deviné se serait donc trompé quinze fois sur seize |
