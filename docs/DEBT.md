...
| DET-NOTES-AMONT | **113 des 116 notes d'impact du catalogue sont `Upstream`** : rédigées à partir d'une description d'une ligne, jamais confrontées au logiciel installé. Trois seulement sont `Verified`, celles de M5b | Donnée | 2 | 2 | 5 | 4 | Créée volontairement avec la dette qu'elle décrit ([ADR-006](adr/ADR-006-catalogue-bloatware-importe.md), D20) : le champ existe pour que ce chiffre soit **lisible** plutôt que dissimulé. Se réduit d'une unité à chaque logiciel réellement observé — c'est le même mouvement que DET-WINDEFAULT, et il se mesure |
