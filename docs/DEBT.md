...
porte plus que **29** — 1 881 → 29 depuis l'ouverture de DET-PROGRAM. **637 tests unitaires
en CI** et 62 Windows, soit 699 — 640 sur le poste de développement, qui porte une capture
...
| DET-FIXTURE-MATERIEL | Les fixtures « synthétiques » ne sont pas fabriquées de rien : ce sont des captures réelles dont les champs identifiants sont lavés. Il reste, dans un dépôt public, 197 tâches planifiées dont 11 nomment des logiciels installés (AMD Ryzen Master, Adobe CCX, NVIDIA App et son GUID), le modèle de carte mère, la version et la date du BIOS. `Versioned_fixtures_are_anonymised` ne peut pas le voir : il ne vérifie qu'un booléen et le préfixe du nom de machine | Test | 2 | 2 | 3 | 12 | Préexistant aux 4 fixtures, relevé en écrivant la 4e. Ce n'est pas une fuite de posture — c'est ce qu'ADR/ARCHITECTURE excluent du dépôt et qui les a fait exclure — mais `ARCHITECTURE.md` décrit `synthetic/` comme « valeurs fabriquées, aucune machine réelle », ce qui est plus fort que le vrai. Deux voies : durcir `synthesise` pour remplacer aussi les listes héritées, ou corriger la phrase. **Ne pas laisser l'écart entre les deux**, c'est lui le problème |
