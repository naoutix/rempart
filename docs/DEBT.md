...
bornés par `global.json`. 792 tests, dont un rejeu de fixtures qui exerce le chemin complet
...
| DET-FICHIERS-MUET | `IFileSystemProvider.ListFiles` rend une liste nue : un dossier de démarrage **refusé** rend exactement ce que rend un dossier vide, et le rapport conclut « aucun autorun » sur une surface que le scan n'a pas pu lire | Code | 2 | 2 | 3 | 12 | La cinquième occurrence de la forme que la phase 2 a fermée quatre fois (DET-WMI-MUET, DET-EXT-MUET, DET-PORTS-MUET, DET-CATALOGUE-MUET). Trouvée en écrivant les tests de `LiveFileSystemProvider` sous DET-WINDOWS-TESTS, et **laissée telle quelle délibérément** : ajouter un canal de statut change ce qu'un instantané stocke (`directories` deviendrait un objet) et c'est une décision à part entière, pas l'effet de bord d'un lot de tests. Le comportement actuel est figé par `A_directory_the_scan_may_not_read_yields_nothing_and_says_nothing`, dont le nom dit ce qu'il fige. À traiter comme les précédentes : le statut **à côté** de la liste, jamais à sa place |
