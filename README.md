...
| `6` | The command line named something nothing declares — a command word, an option, a bare argument, or an option left without its value. Nothing was run: the machine is not the suspect, and the remedy is to retype the line rather than to retry it |
